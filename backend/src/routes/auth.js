...
router.post("/reset-escaladores", async (req, res) => {
  ...
  if (adminSecret !== process.env.JWT_SECRET) return res.status(403).json({ error: "No autorizado" });
  try {
    const hash = await bcrypt.hash(nuevaPassword || 'escalador2026', 12);
    const result = await db("UPDATE usuario SET password_hash = $1 WHERE rol = 'escalador' RETURNING email", [hash]);
    res.json({ message: `${result.rows.length} escaladores actualizados`, emails: result.rows.map(r => r.email) });
  ...
  try {
    ...
    const escResult = await db(
      `INSERT INTO escalador (usuario_id, nombre, apellido, fecha_nacimiento, rango_etario, peso_kg, telefono, contacto_emergencia)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id, nombre, apellido, rango_etario, estado, created_at`,
    ...
    res.status(201).json({ message: "Registro exitoso", usuario: { ...usuario, escalador: escResult.rows[0] }, ...tokens });
  } catch (err) { console.error("Error en registro:", err); res.status(500).json({ error: "Error interno del servidor" }); }
});
...
router.post("/login", [body("email").isEmail(), body("password").notEmpty()], async (req, res) => {
  ...
  try {
    ...
    if (!await bcrypt.compare(password, row.password_hash)) return res.status(401).json({ error: "Credenciales inválidas" });
    const tokens = generateTokens({ id: row.id, email: row.email, rol: row.rol });
    const perfil = { id: row.id, email: row.email, rol: row.rol };
    if (row.esc_id) perfil.escalador = { id: row.esc_id, nombre: row.esc_nombre, apellido: row.esc_apellido, rangoEtario: row.rango_etario, estado: row.esc_estado };
    if (row.ent_id) perfil.entrenador = { id: row.ent_id, nombre: row.ent_nombre, licenciaLey181: row.licencia_ley181 };
...
router.post("/refresh", async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0 || !result.rows[0].activo) return res.status(401).json({ error: "Usuario no válido" });
    res.json(generateTokens(result.rows[0]));
  } catch { res.status(401).json({ error: "Refresh token inválido o expirado" }); }
});

// ─── GET /auth/me ─────────────────────────────────────────
// FIX: ahora incluye inscripciones activas con cohorte completa + pagos
// Esto resuelve el bug donde escalador aparecía "activo" sin grupo.
router.get("/me", require("../middleware/auth").authenticate, async (req, res) => {
  try {
    ...
    if (row.esc_id) {
      // 2. Inscripciones con cohorte completa (la cadena relacional completa)
      ...
      // 3. Pagos de todas sus inscripciones
      ...
      // Agrupar pagos por inscripcion_id
      const pagosByInsc = {};
      for (const p of pagosRes.rows) {
        ...
        pagosByInsc[p.inscripcion_id].push({
          id: p.id, monto: parseFloat(p.monto), fechaPago: p.fecha_pago,
          fechaVencimiento: p.fecha_vencimiento, metodo: p.metodo,
          estado: p.estado, referencia: p.referencia,
        });
      }

      // Armar inscripciones con cohorte anidado
      const inscripciones = inscRes.rows.map(r => ({
        ...
        cohorte: {
          id: r.cohorte_id, modalidad: r.modalidad, horario: r.horario,
          estado: r.cohorte_estado, cupoMaximo: r.cupo_maximo,
          inscritosActual: r.inscritos_actual,
          programa: {
            id: r.prog_id, nombre: r.prog_nombre, nivel: r.nivel,
            descripcion: r.prog_desc, incluyeFisio: r.incluye_fisio,
            incluyeNutricion: r.incluye_nutricion,
          },
          ciclo: {
            id: r.ciclo_id, codigo: r.ciclo_codigo,
